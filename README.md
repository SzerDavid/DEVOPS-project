...
The image is avaialable on [hub.docker.com](https://hub.docker.com) with the following image name: `SzerDavid/DEVOPS-project`.

You can pull the image with the following command.

``` bash
docker pull SzerDavid/DEVOPS-project:latest
```

Or just simply run the app the following way.

``` bash
docker run -d -p 8080:8080 SzerDavid/DEVOPS-project:latest
```
